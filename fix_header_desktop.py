...
<header class="px-6 md:px-12 py-3 flex flex-col lg:flex-row justify-between items-start lg:items-center text-[10px] font-bold uppercase tracking-widest rule-thin sticky top-0 bg-v-white/95 backdrop-blur-sm z-50 text-v-black font-mono-jb gap-4 lg:gap-0">
    
    <!-- Left Side: Brand (Visible on both) -->
    <div class="flex flex-row justify-between lg:justify-start items-center w-full lg:w-auto">
        ...
        <!-- Mobile Right-Side Toggle & Cart (Only visible on mobile) -->
        <div class="flex lg:hidden items-center gap-4">
            ...
            <button id="mobile-nav-toggle" class="p-2 text-v-black focus:outline-none hover:bg-gray-100 rounded">
    ...
    <!-- Right Side: Links & Utility & Cart (Desktop inline, Mobile dropdown) -->
    <nav id="mobile-nav-menu" class="hidden lg:flex flex-col lg:flex-row items-start lg:items-center w-full lg:w-auto gap-4 lg:gap-8 text-v-gray pt-4 lg:pt-0 border-t lg:border-t-0 border-gray-200 lg:border-t-transparent">
        
        <!-- Links -->
        <div class="flex flex-col lg:flex-row gap-4 lg:gap-8 w-full lg:w-auto">
            <a asp-page="/GreenBeans" class="hover:text-v-black transition-colors text-decoration-none">COFFEE</a>
            <a asp-page="/BrewMethods" class="hover:text-v-black transition-colors hidden sm:block text-decoration-none">BREW GUIDES</a>
            <a asp-page="/Experiences" class="hover:text-v-black transition-colors text-decoration-none">EXPERIENCES</a>
            <a asp-page="/Wholesale" class="hover:text-v-black transition-colors hidden md:block text-decoration-none">WHOLESALE</a>
            <a asp-page="/OS" class="hover:text-v-blue text-v-black transition-colors text-decoration-none font-bold">OS</a>
        </div>

        <!-- C-Market Toggle -->
        <button type="button" id="cmarket-toggle" class="group flex items-center gap-1.5 px-2 py-0.5 border border-v-black/20 hover:border-v-black hover:bg-v-black hover:text-v-white transition-all cursor-pointer text-[10px] font-mono-jb uppercase tracking-widest text-v-black mt-2 lg:mt-0" title="Click to toggle between Kilograms and Pounds">
            <span>C-MARKET:</span>
        ...
        <!-- Cart & Bookings for Desktop (Hidden on mobile here since it's up top) -->
        <div class="hidden lg:flex flex-col items-end gap-[3px] ml-0 lg:ml-4">
            <a asp-page="/Cart" class="hover:text-v-red transition-colors text-decoration-none font-bold text-v-black leading-none py-[1px] block">[ CART: @Model ]</a>
            <a asp-page="/Experiences" class="text-v-black flex items-center gap-1.5 leading-none py-[1px] block text-[9px] hover:text-v-green transition-colors text-decoration-none">
                <span class="w-1.5 h-1.5 rounded-full bg-v-green animate-pulse inline-block"></span>
...
with open("Pages/Shared/_Header.cshtml", "w", encoding="utf-8") as f:
    f.write(new_header)
print("Updated _Header.cshtml to fix desktop wrapping")
