...
html_path = r"C:\Users\alext\Downloads\coffee_flavor_explorer (3).html"
...
# It's possible the script match regex above might fail if the structure changed slightly, so let's be broader just in case:
# But we also have the filter script there... Wait!
# Let's find the script block that has 'interactive-canvas'
# I'll just use a more careful removal if it's there.
# Let's just do a regex that captures from `<script>` containing `interactive-canvas` to `</script>`
gb_content = re.sub(r'<script>[^<]*interactive-canvas[^<]*</script>\s*$', '', gb_content, flags=re.DOTALL)
...
print("Updated GreenBeans.cshtml with explorer 3")
